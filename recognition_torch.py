...
for i in range(0, len(x)):
    distance_to_line = abs(slope*x[i] - y[i] + intercept) / np.sqrt(slope**2 + 1)
    if distance_to_line < 10:
        col.append('blue')
        sizes.append(70)
    elif distance_to_line < 20:
        col.append('green')
        sizes.append(40)
    else:
        col.append('magenta')
...
x_long = np.linspace(-90, 90, 100)
plt.plot(x_long, slope * x_long + intercept, 'red', lw=2)
