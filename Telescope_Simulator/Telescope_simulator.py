...
if __name__ == '__main__':
    ...
    input_image_name = "Hubble_Jupiter_image.png"
    ...
    angular_pixel_size_input_image = 0.05
    wavelength = 6000e-9  # in meters
    ...
    # term in denominator is the gridwidth in the pupil plane, image plane psf MUST NOT be cropped Verified this is correct!
    ...
    if atmosphere == True:
        ...
        if int(np.ceil(telescope_aperture_width_pixels)) < 200:
            ...
            # Based on the assumption that pixelSize = 1. Calculates how many pixels wide a distance of the Fried parameter is based on how many Fried paramters wide the sample box is and how many pixels are present in the sample box
            ro = telescope_aperture_width_pixels / (telescope_diameter_m/(r0_cm*0.01))
            # print("Pixels/r0: ", np.round(ro,2))
            # this is the ratio of your telescope size to the Fried parameter
            print("D/r_0: ", np.round(telescope_diameter_m/r0_cm*1e2, 2))
        ...
        # dx is 1.25x the gird width as this ensures that the tweeter screens of each subsequent phase screen are not correlated
        for phaseScreen in M.MegaScreen(numIter=Num_psfs_to_gen, r0=ro, dx=0.5*telescope_aperture_width_pixels, windowShape=(telescope_aperture_width_pixels, telescope_aperture_width_pixels), pixelSize=1):
    ...
    for i in range(0, len(phase_screens)):
        ...
        # for each loop, complex amplitudes is a array of 2D arrays where each 2D array is the propgated complex amplitude from telescope
        complex_amplitude = quick_complex_pupil(phase_screens[i], array_to_propgate_size=len(im_array[0]))
    ...
    convolved_images = []
    # this line carries out a test convolution to get the shape of the convolved arrays for the variable convolved_array
    convolved_array_shape = np.shape(signal.convolve(im_array[:, :, 0], image_plane_psfs[0]*(1/np.max(image_plane_psfs[0]))))
    ...
    for j in range(0, len(convolved_images)):
        ...
        for i in range(0, 3):
            f = interpolate.interp2d(
                x_psf_samples, y_psf_samples, convolved_images[j][:, :, i], kind='cubic')
            temp = f(x_CCD, y_CCD)
            output_image[:, :, i] = temp
    ...
    # palettesize = 256 is the max value. The image countours are due to this being too low
    imageio.mimsave('Animated_images.gif', output_images, format='.gif')
